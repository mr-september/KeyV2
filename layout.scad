    ...
    for(column = column_override ? column_override : [0:len(list[row])-1]) {
      ...
      if (key_length >= 1) {
        translate_u(column_distance - (key_length/2), -row) {
          ...
          key_profile(profile, row_sculpting, column_value) u(key_length) legend(legends ? legends[row][column] : "") legend(topleft_legends ? topleft_legends[row][column] : "", [-1,-1]) legend(botleft_legends ? botleft_legends[row][column] : "", [-1,1]) legend(midright_legends ? midright_legends[row][column] : "", [1,0])  legend(botmid_legends ? botmid_legends[row][column] : "", [-1,0]) legend(midright_legends ? midright_legends[row][column] : "", [1,0]) front_legend(front_legends ? front_legends[row][column] : "") cherry() { // (row+4) % 5 + 1
          $row = row;
          $column = column;
